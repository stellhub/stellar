...
除核心中间件产品外，`核心产品` 下拉菜单中还收纳了一份与全矩阵配套的统一规范文档，用于约束环境变量、请求上下文与指标语义：

<div class="product-grid">
  <a class="product-card" href="/products/observability-spec">
    <span class="product-card-domain">体系规范</span>
    <h3 class="product-card-title">可观测规范</h3>
    <p class="product-card-desc">统一沉淀基础环境变量、全局请求 Header、链路上下文和指标标签边界，作为所有核心产品的观测协同底座。</p>
    <span class="product-card-link">查看规范文档</span>
  </a>
</div>

## 文档结构约定

当前每个中间件目录的首页统一包含以下栏目：
