...
const mockUserServiceAddr = "127.0.0.1:18087"
...
func (s *clientStarter) Name() string {
	return "http-client-interceptor-example"
}
...
func (s *clientStarter) Init(_ context.Context, app *stellar.App) error {
	...
	httpClient, baseURL, err := stellarhttp.NewNamedClientFromConfig(
		s.config.HTTP.Client,
		"user-service",
		app.Observability(),
		stellarhttp.WithClientInterceptors(app.Interceptors()),
	)
	if err != nil {
		return err
	}
...
func (s *clientStarter) Start(ctx context.Context) error {
	...
	s.logger.Info("user loaded",
		...
		"name", user.Name,
		"source", user.Source,
		"interceptor_header", user.InterceptorHeader,
	)
	return nil
}
