...
const DefaultDBName = "postgresql"
...
func NewDBFromConfig(ctx context.Context, cfg *config.PostgreSQLConfig, provider *observability.Provider) (*DB, error) {
	if cfg == nil {
		return nil, fmt.Errorf("stellar: postgresql config is required")
	}
	if strings.TrimSpace(cfg.DSN) == "" {
		return nil, fmt.Errorf("stellar: postgresql dsn is required")
	}
	if provider == nil {
		provider = observability.New()
	}

	driver := valueOrDefault(cfg.Driver, "pgx")
	...
	if provider.PostgreSQLClientMetricsEnabled() {
	...
	db := &DB{
		...
		driver:            driver,
		provider:          provider,
		logsEnabled:       provider.PostgreSQLClientLogsEnabled(),
	...
	if cfg.PingOnStartup {
		...
		if cfg.PingTimeout != "" {
			...
			if err != nil {
				_ = db.Close()
				return nil, fmt.Errorf("stellar: invalid postgresql ping_timeout %q: %w", cfg.PingTimeout, err)
			}
...
func (db *DB) emit(ctx context.Context, operation string, duration time.Duration, err error) {
	...
	db.provider.EmitPostgreSQLClientLog(ctx, observability.PostgreSQLClientLog{
		Operation: operation,
		Driver:    db.driver,
...
func openDB(driver string, dsn string, provider *observability.Provider) (*sql.DB, error) {
	if !provider.PostgreSQLClientTraceEnabled() && !provider.PostgreSQLClientMetricsEnabled() {
	...
	if provider.PostgreSQLClientTraceEnabled() {
...
func applyPoolConfig(db *sql.DB, cfg *config.PostgreSQLConfig) error {
	...
	if cfg.ConnMaxLifetime != "" {
		...
		if err != nil {
			return fmt.Errorf("stellar: invalid postgresql conn_max_lifetime %q: %w", cfg.ConnMaxLifetime, err)
		}
	...
	if cfg.ConnMaxIdleTime != "" {
		...
		if err != nil {
			return fmt.Errorf("stellar: invalid postgresql conn_max_idle_time %q: %w", cfg.ConnMaxIdleTime, err)
		}
